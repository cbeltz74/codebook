    ...
    { best_seller && <span className="absolute top-4 right-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded">Best Seller</span> }
        ...
        <div className="p-5">
            ...
            <p className="flex justify-between items-center">
                ...
                { inCart && <button onClick={() => removeFromCart(product)} className={`inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 ${product.in_stock ? "" : "cursor-not-allowed"}`} disabled={ product.in_stock ? "" : "disabled" }>Remove Item <i className="ml-1 bi bi-trash3"></i></button> }
            </p>
        </div>
    </div>
  )
